...
<button type="button" class="btn btn-lg btn-primary" onclick="payment();" style="margin-bottom:0px;">
...
<div class="col-lg-6">
<div class="panel panel-custom panel-border" style="background-color:#373a36;">

<div class="panel-body" style="padding:0px;">

<div class="row"  style="background-color:#fff; margin-bottom:10px; border-radius:5px; padding:0px;">
	...
	<div class="col-lg-12 col-md-12 col-sm-12 ">
			<div class="card-box" style="margin-bottom:10px; padding:10px;">
				<div class="portfolioFilter">
					<a href="javascript:void(0)" data-bind="click:allItems" class="btn btn-primary  current btn-default waves-effect waves-custom" >All</a>
...
<div class="portfolioContainer" data-bind="foreach:items">
	<div class="col-sm-6 col-lg-3 col-md-4 sides-salads pizza-farm">
		<div class="gal-detail thumb">
			...
			<p class="text-muted" data-bind="text:'AED '+parseFloat($data.price).toFixed(2)">10.5</p></a>
		</div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" onclick="confirmForCart();" class="btn btn-info waves-effect waves-light">Confirm</button>
                </div>
                <!-- /.modal-content -->
            </div>
        </div>
	...
	<div id="discount" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
            <div class="modal-content">
				...
				<div class="modal-footer">
                    <button type="button" class="btn btn-default btn-info waves-effect" data-dismiss="modal">Cancel</button>
					<button type="button" class="btn btn-default btn-info waves-effect" onclick="applyDiscount();">OK</button>
                </div>
			</div>
			</div>	
	</div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
					<div id="pay_sec" class="row" >
						<button class="btn btn-default waves-effect waves-light btn-lg m-b-5 btn-size" onclick="setPay('Cash',this)">Cash</button>
					
						<button class="btn btn-default waves-effect waves-light btn-lg m-b-5 btn-size" onclick="setPay('Card',this)">Card</button>
		
					</div>
				</div>
				<div class="modal-footer">
                    <button type="button" class="btn btn-default btn-info waves-effect" data-dismiss="modal">Cancel</button>
					<button type="button" class="btn btn-default btn-info waves-effect" onclick="order();">OK</button>
                </div>
			</div>
			</div>	
	</div>
...
function PosModel() {
	...
	self.ItemsByCategory=function(index,category){
		...
		$.get('<?php echo $this->request->getAttribute("webroot"); ?>api/items',{id:category.id},function(data){
			pm.items(JSON.parse(data));
			$('.portfolioFilter a').removeClass('btn-primary  current');
			$('.portfolioFilter a').eq(index+1).addClass('btn-primary  current');
			$('#loader').addClass('hide');
		});
	}
	self.allItems=function(){
		...
		$.get('<?php echo $this->request->getAttribute("webroot"); ?>api/items',{brand_id:pm.brands()[pm.selectedBrand()].id},function(data){
			pm.items(JSON.parse(data));
			$('.portfolioFilter a').removeClass('btn-primary  current');
			$('.portfolioFilter a').eq(0).addClass('btn-primary  current');
			$('#loader').addClass('hide');
		});
	}
	self.addTo=function(index,item){
		if(item.item_sizes.length>0){
			...
			$('.curstbtn').removeClass('btn-primary');
			$('.error').remove();
	...
	self.selectSize=function(index,item){
		$('.btn-size').removeClass('btn-primary');
		$('.btn-size').eq(index).addClass('btn-primary');
...
function cancelOrder(){
 ...
 function setDiscount(dis,obj){
	 pm.discount(dis);
	 $('#disc button').removeClass('btn-primary');
	 $(obj).addClass('btn-primary');
 }
 ...
 function setPay(pay,obj){
	 $('#pay_sec button').removeClass('btn-primary');
	 $(obj).addClass('btn-primary');
 }
