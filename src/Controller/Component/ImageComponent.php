...
class ImageComponent extends Component
{
	...
	*/	
	function upload_image_and_thumbnail($data, $maxw, $maxh, $thumbscalew, $thumbscaleh, $folderName) {
		...
		if (strlen($data['name'])>4){ 
					...
					$originaluploaddir = "img/".$folderName."/original"; // the /original/ directory for original image
                    ...
                    {                    
                        ...
                        if (!copy($data['tmp_name'],"$tempfile"))
                        {
							 ...
							 */
							 move_uploaded_file($data['tmp_name'], $originalfile);
							 //$this->resizeImage('', $tempuploaddir, $filename, $homeuploaddir, $filename, '', '', 100);
							$this->resizeImage('resizeCrop', $tempuploaddir, $filename, $homeuploaddir, $filename, 886, 473, 100);
							/*
							 *	Generate the big version of the image with max of $imgscale in either directions
							 */
							$this->resizeImage('resize', $tempuploaddir, $filename, $biguploaddir, $filename, $maxw, $maxh, 100);						

							/*
							 *	Generate the small thumbnail version of the image with scale of $thumbscalew and $thumbscaleh
							 */
							$this->resizeImage('resizeCrop', $tempuploaddir, $filename, $smalluploaddir, $filename, $thumbscalew, $thumbscaleh, 100);
	...
	*/
	function delete_image($filename,$folderName) {
		...
		if(is_file("img/".$folderName."/original/".$filename))
